...
export async function executeRenderJob(
  ...
  try {
    ...
    // ── Native render path (optional) ─────────────────────────────────
    // When the native renderer is available and the composition is supported,
    ...
    const nativeRenderMode = process.env.HYPERFRAMES_NATIVE_RENDER;
    const useNativeRenderer = Boolean(nativeRenderMode);
    const pixelPerfectMode = nativeRenderMode === "pixel-perfect";
    if (useNativeRenderer && outputFormat === "mp4") {
      ...
      const nrResolve = (p: string) => {
        const candidates = [
          join(dirname(new URL(import.meta.url).pathname), "../../../native-renderer/src", p),
          join(process.cwd(), "packages/native-renderer/src", p),
        ];
      ...
      const findNativeRendererRoot = (): string | null => {
        const candidates = [
          join(dirname(new URL(import.meta.url).pathname), "../../../native-renderer"),
          join(process.cwd(), "packages/native-renderer"),
        ];
        for (const c of candidates) {
          if (existsSync(join(c, "Cargo.toml")) && existsSync(join(c, "src/bin/render_native.rs")))
            return c;
        }
        return null;
      };
      ...
      if (pixelPerfectMode) {
        ...
        for (let t = 0; t <= dur + 0.001; t += 1 / job.config.fps) {
          const rounded = Math.round(t * 1000) / 1000;
          if (isInVideoRange(rounded)) {
            const key = rounded.toFixed(3);
            if (!seen.has(key)) {
              stateTimes.push(rounded);
              seen.add(key);
            }
          }
        }
        for (let t = 0; t <= dur; t += 2.5) {
          const rounded = Math.round(t * 1000) / 1000;
          const key = rounded.toFixed(3);
          if (!isInVideoRange(rounded) && !seen.has(key)) {
            stateTimes.push(rounded);
            seen.add(key);
          }
        }
        if (!seen.has("0.000")) stateTimes.unshift(0);
        if (!seen.has(dur.toFixed(3))) stateTimes.push(dur);
        stateTimes.sort((a, b) => a - b);
      } else {
        const numSparse = Math.min(Math.max(15, Math.ceil(dur / 2.5)), 80);
        for (let i = 0; i < numSparse; i++) {
          stateTimes.push((dur * i) / (numSparse - 1));
        }
      }

      const numStates = stateTimes.length;
      const videoStateCount = stateTimes.filter(isInVideoRange).length;
      log.info("State capture plan", {
        mode: pixelPerfectMode ? "pixel-perfect" : "fast",
        numStates,
        videoStates: videoStateCount,
      ...
      log.info("Captured states", {
        mode: pixelPerfectMode ? "pixel-perfect" : "fast",
        count: stateFramePaths.length,
      });

      // Build scene: Voronoi-tiled state images
      ...
      let sceneChildren: Record<string, unknown>[] = stateChildren;

      // Fast mode: add video overlay children with baked timeline metadata
      if (!pixelPerfectMode && videoOverlays.length > 0) {
        ...
        const findStyle = (id: string): Record<string, unknown> => {
        ...
        const videoOverlayChildren = [...deduped.values()].map((v) => {
          const ex = findStyle(v.id);
          return {
            id: v.id,
            ...
            style: {
              ...
              data_end: v.dataEnd,
              object_fit: ex.object_fit ?? "cover",
              object_position: ex.object_position ?? undefined,
              mix_blend_mode: ex.mix_blend_mode ?? undefined,
            } as Record<string, unknown>,
            children: [] as Record<string, unknown>[],
          };
        });

        sceneChildren = [...stateChildren, ...videoOverlayChildren];
      }
      ...
      log.info("Scene built", {
        mode: pixelPerfectMode ? "pixel-perfect" : "fast",
        states: stateChildren.length,
        videoOverlays: sceneChildren.length - stateChildren.length,
      });
      ...
      if (!nativeRoot) throw new Error("Native renderer root not found");

      updateJobStatus(job, "rendering", "Native render: Skia painting frames", 45, onProgress);

      const { execFileSync } = await import("node:child_process");
      const binaryPath = join(nativeRoot, "target/release/render_native");
      if (!existsSync(binaryPath)) {
        log.info("Building native renderer binary...");
      ...
      // Collect audio and jump to Stage 6 (assemble)
      if (audioPromise) {
      ...
      if (hasAudio) {
        ...
        assertNotAborted();
        if (!muxResult.success) throw new Error(`Audio muxing failed: ${muxResult.error}`);
      } else {
        const fsResult = await applyFaststart(videoOnlyPath, outputPath, abortSignal);
        assertNotAborted();
        if (!fsResult.success) throw new Error(`Faststart failed: ${fsResult.error}`);
      }
      ...
      job.perfSummary = {
        ...
        stages: perfStages,
        tmpPeakBytes,
        peakRssBytes: peakRssBytes || undefined,
        peakHeapUsedBytes: peakHeapUsedBytes || undefined,
      } as RenderPerfSummary;
      writeFileSync(perfOutputPath, JSON.stringify(job.perfSummary, null, 2));
    }

    // Skip Chrome capture + encode when native render already produced the output.
    const nativeRenderCompleted = useNativeRenderer && existsSync(outputPath);
    let workerCount = 1;
    if (!nativeRenderCompleted) {
      ...
      workerCount = calculateOptimalWorkers(totalFrames, job.config.workers, cfg);
              ...
              if (activeTransition && transBufferA && transBufferB && transOutput) {
                ...
                for (const [sceneBuf, sceneIds] of [
                  ...
                  try {
                    ...
                    log.warn(
                      "DOM layer decode/blit failed; skipping overlay for transition scene",
                      {
                        frameIndex: i,
