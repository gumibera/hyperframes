...
- Instructions: "Write your output to `.hyperframes/scenes/sceneN.html` as a single file with three clearly marked sections: `<!-- HTML -->`, `<!-- CSS -->`, `<!-- GSAP -->`. Use the timeline variable `tl`. Start tweens at `{start_time}`. Do not create the timeline or register it — the parent handles that. **Do NOT use `tl.from()` in multi-scene compositions** — use `tl.set()` + `tl.to()` instead. The `tl.set` hides the element at the scene start time, then `tl.to` animates it in. Example: `tl.set('#el', { opacity: 0, y: 30 }, sceneStart); tl.to('#el', { opacity: 1, y: 0, duration: 0.4 }, sceneStart + 0.5);`. The `tl.from` approach causes elements to flash visible at their CSS default state before the entrance tween fires. **Do NOT set `position`, `top`, `left`, `width`, `height`, `opacity`, or `z-index` on the `#sceneN` container in your CSS** — the scaffold owns those properties. Only style elements INSIDE the scene."
