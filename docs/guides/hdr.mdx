...
<Steps>
  <Step title="Add an HDR source to your composition">
    ...
    - **HDR video** tagged BT.2020 with PQ (`smpte2084`) or HLG (`arib-std-b67`) transfer — for example, the original MP4 you get from [yt-dlp](https://github.com/yt-dlp/yt-dlp) when downloading a YouTube HDR video at the highest quality
...
A common HDR source is YouTube HDR. Download the highest-quality stream with `yt-dlp` and Hyperframes will detect it automatically:

```bash Terminal
yt-dlp -f "bestvideo[dynamic_range=HDR]+bestaudio" -o assets/clip.mp4 <url>
