...
<Tabs>
  ...
  <Tab title="Utilities">
    ...
    Skills are fetched from the HyperFrames docs site and include composition authoring, GSAP animation patterns, registry block/component wiring, and other domain-specific knowledge. The `init` command also offers to install skills automatically after scaffolding a project.

    The hosted install path avoids `git clone`, so it works on machines without `git-lfs` and avoids clone-hook failures from globally installed Git LFS hooks.

    #### Direct install

    If you want to install the skills without going through the HyperFrames CLI, use the hosted source directly:

    ```bash
    npx skills add https://hyperframes.heygen.com
    ```

    The legacy repo source (`npx skills add heygen-com/hyperframes`) still depends on the upstream `skills` CLI clone path and may require future upstream fixes for Git LFS-free installs.
  </Tab>
</Tabs>
